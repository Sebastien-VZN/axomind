...
- **Messages, fichiers et clips audio** : Ces données sont générées par l'Utilisateur. L'intégralité de ces contenus est chiffrée au repos sur les serveurs de l'Auteur (AES-256-GCM avec AAD) et chiffrée en transit entre l'appareil de l'Utilisateur et l'infrastructure de l'Auteur (TLS). Les clés de chiffrement sont détenues par l'infrastructure serveur de l'Auteur.

L'Auteur n'a déployé aucun outil de consultation, d'administration ou de lecture en clair des contenus des Utilisateurs. L'Auteur n'accède pas au contenu des communications. En cas de réquisition judiciaire dûment fondée, l'Auteur coopère avec les autorités compétentes conformément à la loi. Les données d'identification (compte, journaux de connexion, métadonnées) peuvent être communiquées dans ce cadre. Le contenu chiffré des communications nécessite une procédure légale spécifique pour tout traitement, dans le respect du cadre juridique applicable.
...
- **Chiffrement des contenus** : Toutes les communications, messages, fichiers et clips audio sont chiffrés en transit (TLS entre l'appareil de l'Utilisateur et l'infrastructure de l'Auteur) et au repos (AES-256-GCM avec AAD). Les clés de chiffrement au repos sont détenues par l'infrastructure serveur. L'Auteur n'a déployé aucun outil de lecture en clair des contenus utilisateurs.
- **Transit des données** : Les données sont chiffrées entre l'appareil de l'Utilisateur et l'infrastructure de l'Auteur via TLS. L'Auteur peut recourir à un service de proxy inverse tiers pour la terminaison TLS. Le choix de ce service peut évoluer dans le temps.
...
- **Messages, files, and audio clips**: This data is generated by the User. All content is encrypted at rest on the Author's servers (AES-256-GCM with AAD) and encrypted in transit between the User's device and the Author's infrastructure (TLS). The encryption keys are held by the Author's server infrastructure.

The Author has not deployed any tool for viewing, administering, or reading the plaintext content of Users' data. The Author does not access the content of communications. In the event of a duly founded judicial requisition, the Author cooperates with the competent authorities in accordance with the law. Identification data (account, connection logs, metadata) may be communicated in this context. The encrypted content of communications requires a specific legal procedure for any processing, in compliance with the applicable legal framework.
...
- **Content encryption**: All communications, messages, files, and audio clips are encrypted in transit (TLS between the User's device and the Author's infrastructure) and at rest (AES-256-GCM with AAD). The encryption keys at rest are held by the server infrastructure. The Author has not deployed any plaintext reading tool for user content.
- **Data transit**: Data is encrypted between the User's device and the Author's infrastructure via TLS. The Author may use a third-party reverse proxy service for TLS termination. The choice of this service may evolve over time.
